...
def _extract_command(model_response: str) -> str | None:
    match = _COMANDO_RE.search(model_response)
    if not match:
        return None
...
def _confirm_execution(command_text: str) -> bool:
...
def main() -> int:
    ...
    while True:
        ...
        model_response = agente.ask(user_input)
        print(f"\nagente> {model_response}\n")
        logger.info("SOLICITUD | usuario=%s | texto=%r", usuario, user_input)

        # 2. ¿Propuso un comando?
        proposed_command = _extract_command(model_response)
        if not proposed_command:
            continue

        # 3. Confirmación humana.
        if not _confirm_execution(proposed_command):
            print("[CANCELADO] No se ejecuto el comando.")
            logger.info("EJECUCION_CANCELADA | usuario=%s | comando=%r", usuario, proposed_command)
            continue

        # 4. Validar de nuevo y ejecutar.
        try:
            execution_result = run_command(proposed_command)
            print(f"\n[RESULTADO]\n{execution_result}\n")
            logger.info(
                "EJECUCION_OK | usuario=%s | comando=%r | bytes_salida=%d",
                usuario, proposed_command, len(execution_result),
            )
        except SecurityError as exc:
            print(f"[RECHAZADO POR SEGURIDAD] {exc}")
            logger.warning(
                "EJECUCION_RECHAZADA | usuario=%s | comando=%r | motivo=%s",
                usuario, proposed_command, exc,
            )
